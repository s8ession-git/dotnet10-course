...
-- 6
select c.id, count(c.id) booking_count
from bookings b 
...
-- 9
select bi.event_id, sum(bi.ticket_count) total_tickets, count(bi.ticket_count * bi.unit_price) confirmed_total 
from booking_items bi  
join bookings b on b.id = bi.booking_id 
...
-- 10
select c.id, count(distinct(b.id)) booking_count, count(coalesce(bi.ticket_count, 0)) total_tickets, sum(coalesce(bi.unit_price, 0))
from bookings b 
join booking_items bi on b.id = bi.booking_id
